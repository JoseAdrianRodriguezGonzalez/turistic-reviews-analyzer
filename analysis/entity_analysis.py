...
def _enriquecer_entidad(
    ...
    if len(indices_validos) < Params.MIN_DOCS_ENTIDAD:
        return None
...
def calcular_entidades_con_sentimiento(
    ...
    indice_a_fila = dict(zip(df_corpus['indice'].astype(int), range(len(df_corpus))))

    logger.info(
        'Procesando %d entidades (mínimo %d documentos)...',
        len(ner_grupos),
        Params.MIN_DOCS_ENTIDAD,
    )
...
def run_entity_analysis() -> dict[str, pd.DataFrame]:
    '''
    ...
    ner_grupos = _cargar_ner_groups(Paths.NER_GROUPS_JSON)
    df_corpus  = _cargar_corpus_sentimiento(Paths.CORPUS_CON_SENTIMIENTO_CSV, Paths.DOCS_WITH_TOPICS_CSV)

    df_entidades = calcular_entidades_con_sentimiento(
        ner_grupos[:Params.MAX_ENTIDADES],
        df_corpus,
    )
    ...
    logger.info('=== Análisis de entidades completado. Archivos en: %s ===', Paths.ENTITIES_DIR)
