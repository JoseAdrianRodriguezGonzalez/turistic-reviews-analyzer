...
def build_linkage_matrix(
    ...
    if method is None:
        method = Params.HIERARCHY_LINKAGE_METHOD
    if metric is None:
        metric = Params.HIERARCHY_LINKAGE_METRIC
    '''
    ...
    # pdist es O(n²) en RAM — subsamplear si el corpus es demasiado grande
    if len(X) > Params.HIERARCHY_MAX_DOCS:
        rng = np.random.default_rng(Params.RANDOM_STATE)
        idx = rng.choice(len(X), size=Params.HIERARCHY_MAX_DOCS, replace=False)
        X = X[idx]
        logger.info(
            'Corpus grande — linkage calculado sobre muestra de %d docs',
            Params.HIERARCHY_MAX_DOCS,
        )
