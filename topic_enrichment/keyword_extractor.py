...
def _build_bow_matrix(
    ...
    '''
    Construye la matriz Bag of Words sparse para un subconjunto del corpus.

    Usa scipy.sparse.csr_matrix en lugar de numpy denso:
    memoria O(n_docs × avg_tokens) vs O(n_docs × vocab_size).
    Para un cluster de 2K docs con vocabulario de 50K esto pasa de
    ~400 MB a ~80 KB.
    ...
    Retorna:
        Matriz CSR sparse de shape (n_docs_cluster, vocab_size)
    '''
    rows, cols, data = [], [], []
    for i, documento in enumerate(corpus_cluster):
        ...
        for token in documento.strip().split():
    ...
    return sp.csr_matrix((data, (rows, cols)), shape=(n_docs, vocab_size), dtype=np.int32)


def _calcular_tf(bow: sp.csr_matrix) -> sp.csr_matrix:
    '''
    ...
    Trabaja con matrices sparse mediante multiplicación por diagonal inversa.
    '''
    totales = np.asarray(bow.sum(axis=1)).ravel()
    totales_safe = np.where(totales > 0, totales, 1.0)
    # Multiplicar cada fila i por 1/totales_safe[i]
    tf = sp.diags(1.0 / totales_safe) @ bow
    return tf


def _calcular_idf(bow: sp.csr_matrix) -> np.ndarray:
    '''
    ...
    '''
    n_docs = bow.shape[0]
    df = np.asarray((bow > 0).sum(axis=0)).ravel()
...
def compute_tfidf_per_cluster(
    ...
    for cluster_id in cluster_ids:
        ...
        # Construir BoW sparse interno del cluster
        bow = _build_bow_matrix(corpus_cluster, vocab_index, vocab_size)

        # Calcular TF-IDF interno (todo sparse hasta el promedio final)
        tf    = _calcular_tf(bow)
        idf   = _calcular_idf(bow)
        tfidf = tf.multiply(idf)  # broadcast idf (1D) por columnas sobre sparse tf

        # Score promedio por término — convertir a array 1D
        score_promedio = np.asarray(tfidf.mean(axis=0)).ravel()

        # Document frequency dentro del cluster
        df_cluster = np.asarray((bow > 0).sum(axis=0)).ravel().astype(int)
