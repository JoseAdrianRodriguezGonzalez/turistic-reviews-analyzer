    ...
    for path in paths:
        ...
        for row in data:
            ...
            ents = [
                ...
                if len(e["text"]) >= Params.NER_MIN_ENTITY_LEN
            ]
...
def _enrich_texts_with_ner(df: pd.DataFrame, doc_entities: dict[int, list[str]]) -> list[str]:
    enriched_texts = []
    for idx, text in zip(df["indice"], df[Params.COLUMNA_TEXTO]):
...
def pipe() -> None:
    ...
    top_words = get_top_tfidf_words(features["X_tfidf"], features["vectorizer"], top_k=Params.SEMANTIC_TFIDF_TOP_WORDS)
...
def pipe_microtopics() -> None:
    ...
    for region in tqdm(df["location"].unique(), desc="Regions"):
        for topic in tqdm(df["topic"].unique(), desc=f"Topics {region}", leave=False):
            ...
            if len(subset) < Params.MICROTOPIC_MIN_DOCS:
                continue
