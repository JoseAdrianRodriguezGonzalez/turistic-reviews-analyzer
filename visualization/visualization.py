...
try:
    ...
    INPUT_DIR = str(Path(Paths.PROCESSED_DIR).parent / "analysis" / "sentiment_topics")
    OUTPUT_FILE = str(Paths.RESULTS_DIR / "reporte_interactivo.html")
except ImportError:
    print("No se encontró config.py. Usando rutas relativas de respaldo")
    INPUT_DIR = "C:/tmp/st/sentiment_topics"
    OUTPUT_FILE = str(Path(__file__).resolve().parent / "reporte_interactivo.html")
