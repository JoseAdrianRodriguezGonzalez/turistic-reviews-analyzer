...
def cosine_similarity_to_centroid(
    ...
    '''
    Calcula la similitud coseno de cada documento del cluster con su centroide.
    Usa operaciones vectorizadas numpy en lugar de un loop Python por documento.
    '''
    vectores = X[indices_cluster]
    normas = np.linalg.norm(vectores, axis=1)
    norma_c = float(np.linalg.norm(centroide))

    if norma_c == 0.0:
        return [(int(idx), 0.0) for idx in indices_cluster]

    centroide_norm = centroide / norma_c
    # Evitar división por cero para vectores nulos
    normas_safe = np.where(normas > 0, normas, 1.0)
    vectores_norm = vectores / normas_safe[:, np.newaxis]
    sims = vectores_norm @ centroide_norm
    # Documentos con norma 0 reciben similitud 0
    sims = np.where(normas > 0, sims, 0.0)

    pares = [(int(indices_cluster[i]), float(sims[i])) for i in range(len(indices_cluster))]
