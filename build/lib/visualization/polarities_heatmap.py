...
# Dict de etiquetas de destino construido desde Params
...
def run_polarities_heatmap() -> None:
    ...
    top_topics = topic_docs[topic_docs >= Params.VIZ_HEATMAP_MIN_DOCS].nlargest(Params.VIZ_POLARITIES_TOP_N).index.tolist()
    ...
    df_pivot.columns = [_DEST_LABELS.get(c, c) for c in df_pivot.columns]
    ...
    df_ndocs.columns = [_DEST_LABELS.get(c, c) for c in df_ndocs.columns]
    ...
    fig.update_layout(
        title=dict(
            text=(
                f"Polaridad de Sentimiento: Top {Params.VIZ_POLARITIES_TOP_N} Tópicos × Destino<br>"
