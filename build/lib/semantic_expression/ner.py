...
def clean_entities(data: list[dict]) -> list[dict]:
    valid = frozenset(Params.NER_ENTITY_TYPES)
    cleaned = []
    for row in data:
        index = row["indice"]
        for entity in row["entities"]:
            if entity["label"] not in valid:
                continue
            if len(entity["text"]) < Params.NER_MIN_ENTITY_LEN:
                continue
            cleaned.append({
                "index": index,
...
def merge_similar_entities(
    ...
    if threshold is None:
        threshold = Params.NER_MERGE_THRESHOLD
    merged = []
...
def enrichment_text(
    ...
    if top_k is None:
        top_k = Params.NER_TOP_NOUN_PHRASES
