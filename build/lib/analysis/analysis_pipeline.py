...
def _generar_resumen_consolidado(
    ...
    if not df_resumen.empty:
        path_resumen = Paths.ANALYSIS_RESUMEN_CSV
...
def run_analysis_pipeline(
    ...
    Ejecuta en orden los seis módulos de análisis y genera un
    ...
    Parámetros:
        ...
        ejecutar_outliers         -- si True, ejecuta detección de outliers
        ejecutar_sentiment_topics -- si True, ejecuta modelado de tópicos por sentimiento
    ...
    Retorna diccionario anidado:
        {
            ...
            'outliers'         : {outliers, normales, unigrams, bigrams, trigrams, resumen},
            'sentiment_topics' : {positivo, negativo, precio_valor_costo},
        }
    '''
    ...
    logger.info(
        'Módulos activos: sentiment=%s | entities=%s | cooccurrence=%s | trends=%s | outliers=%s | sentiment_topics=%s',
    ...
    if ejecutar_trends:
        logger.info('\n--- Paso 4/5: Detección de tendencias ---')
        t0 = time.time()
        try:
    ...
    if resultados:
        ...
        _generar_resumen_consolidado(resultados, Paths.ANALYSIS_DIR)
    ...
    logger.info('Archivos en: %s', Paths.ANALYSIS_DIR)

    archivos_generados = list(Paths.ANALYSIS_DIR.rglob('*.csv'))
    logger.info('\nArchivos generados: %d', len(archivos_generados))
    for archivo in sorted(archivos_generados):
        ruta_relativa = archivo.relative_to(Paths.ANALYSIS_DIR)
