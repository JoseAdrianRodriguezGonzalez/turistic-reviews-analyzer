...
def _build_ngrams_and_frequency(tokenized_texts: list[str], n: int) -> pd.DataFrame:
    ...
    df = pd.DataFrame(frequency_count.most_common(), columns=['ngram', 'total_frequency'])
    df['relative_frequency'] = (df['total_frequency'] / total_ngrams).round(6)
    return df
