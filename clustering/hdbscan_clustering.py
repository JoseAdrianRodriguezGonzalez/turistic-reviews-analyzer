...
def silhouette_score_safe(X: np.ndarray, etiq: np.ndarray) -> float | None:
    ...
    Para n > _SILHOUETTE_UMBRAL usa muestreo aleatorio (O(n²) → O(k²))
    con k=_SILHOUETTE_SAMPLES; el error de estimación converge por debajo del 1%.
    Retorna None si el cálculo falla (p. ej. todos los puntos en un cluster).
    '''
    from sklearn.metrics import silhouette_score
    try:
        if len(X) > _SILHOUETTE_UMBRAL:
            idx = _RNG.choice(len(X), size=_SILHOUETTE_SAMPLES, replace=False)
