...
# Para n > este umbral se aproxima silhouette con muestreo.
# silhouette_score es O(n²) en distancias; con 5K samples la estimación
# converge (<1% error) y cuesta O(5000²) = 25M ops en lugar de O(54K²) = 3B ops.
_SILHOUETTE_UMBRAL  = 10_000
_SILHOUETTE_SAMPLES = 5_000


def _silhouette_aproximado(X: np.ndarray, etiq: np.ndarray, rng: np.random.Generator) -> float:
    '''Calcula silhouette sobre una muestra aleatoria cuando n > _SILHOUETTE_UMBRAL.'''
    if len(X) > _SILHOUETTE_UMBRAL:
        idx = rng.choice(len(X), size=_SILHOUETTE_SAMPLES, replace=False)
...
def evaluar_kmeans(
    ...
    if len(X) > _SILHOUETTE_UMBRAL:
        logger.info(
            ...
            len(X), _SILHOUETTE_UMBRAL, _SILHOUETTE_SAMPLES,
        )
